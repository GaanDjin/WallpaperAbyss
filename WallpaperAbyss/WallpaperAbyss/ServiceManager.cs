...
{
    /// <summary>
    /// http://www.c-sharpcorner.com/article/installing-a-service-programmatically/
    /// 
    /// Installs and Uninstalls the WallpaperAbyssDesktopBackgroundService from the config window.
    /// </summary>
    public class ServiceManager
    {
        ...
        //private static bool ShowLoginPrompt()
        //{
        ...
        public static bool InstallService(string svcPath, string svcName, string svcDispName)
        {
            ...
            try
            {
                ...
                if (sc_handle.ToInt32() != 0)
                {
                    ...
                    CredentialsPrompt.GetCredentialsVistaAndUp("Service Account", out creds);
                    string lpServiceStartName = creds.UserName;
                    string lpPassword = creds.Password;

                    if (!lpServiceStartName.Contains("\\"))
                        lpServiceStartName += ".\\" + lpServiceStartName;

                    IntPtr sv_handle = CreateService(sc_handle, svcName, svcDispName, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, svcPath, null, 0, null, lpServiceStartName, lpPassword);
